...
typedef enum vulkan_device_support_flag_bits {
	...
	// Indicats if the device supports native dynamic topology (i.e. using Vulkan API >= 1.3)
	...
	VULKAN_DEVICE_SUPPORT_FLAG_LINE_SMOOTH_RASTERIZATION_BIT = 0x04,
...
// Bitwise flags for device support. See vulkan_device_support_flag_bits
typedef u32 vulkan_device_support_flags;

typedef struct vulkan_device {
	// The supported device-level api (major, minor, patch)
	u32 api_major;
	u32 api_minor;
	u32 api_patch;
...
typedef struct vulkan_context {
	// The instance-level api(major, minor, patch)
	u32 api_major;
	u32 api_minor;
	u32 api_patch;
