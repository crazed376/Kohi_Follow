...
// NOTE: To disable this custom allocator, comment this out or set to 0
...
/**
 * @brief Implementation of PFN_vkAllocationFunciont
 * @link https://www.khronos.org/registry/vulkan/specs/1.3-extensions/man/htm/PFN_vkAllocationFuncion.html
 * 
 ...
 * @param allocation_scope The allocation scope and lifetime
 ...
 * @link https://www.khronos.org/registry/vulkan/specs/1.3-extensions/man/html/PFN_vkFreeFuncion.html
 * 
 * @param user_data User data specified in the allocator by the application
...
#endif
        // Free the orignal memory only if the new allocation was successful
 ...
 * 
 * @param pUserDate user data specified in the allocator by the application
...
/**
 * @brief Create a vulkan allocator object, filling out the function pointers in provided struct
 * 
    ...
    for(u32 i=0; i<required_extension_count; ++i) {
        ...
        for(u32 j=0; j<available_extension_count; ++j) {
            if(strings_equal(required_extensions[i], available_extensions[j].extensionName)) {
                found = true;
                KINFO("Required extension found: %s...", required_extensions[i]);
                break;
            }
        }
...
// If validation should be done, geta list of the required validation layer names
...
b8 vulkan_renderer_renderpass_begin(renderer_plugin* plugin, renderpass* pass, render_target* target) {
    ...
    if(do_clear_color) {
        kcopy_memory(clear_values[begin_info.clearValueCount].color.float32, pass->clear_color.elements, sizeof(f32) * 4);
...
void vulkan_renderer_texture_create(renderer_plugin* plugin, const u8* pixels, texture* t) {
    ...
    // NOTE: Lots of assumptions here, different texture types will
    // require different options here
...
void vulkan_renderer_texture_resize(renderer_plugin* plugin, texture* t, u32 new_width, u32 new_height) {
    ...
    if(t && t->internal_data) {
        ...
        // Data is not preserved because there's no reliable way to map the old
        // data to the new since the amount of data differs
        vulkan_image* image = (vulkan_image*)t->internal_data;
        vulkan_image_destroy(context, image);
...
void vulkan_renderer_texture_write_data(renderer_plugin* plugin, texture* t, u32 offset, u32 size, const u8* pixels) {
    ...
    vulkan_command_buffer_begin_single_use(context, pool, &temp_buffer);
...
void vulkan_renderer_shader_destroy(renderer_plugin* plugin, shader* s) {
    ...
    if(s && s->internal_data) {
        ...
        for(u32 i=0; i<shader->config.stage_count; ++i) {
            vkDestroyShaderModule(logical_device, shader->stages[i].handle, vk_allocator);
        }
...
b8 vulkan_renderer_shader_initialize(renderer_plugin* plugin, shader* s) {
    ...
    // probably be configured to pull from someplace else.
    // Viewport
    VkViewport viewport;
    viewport.x = 0.0f;
    ...
    VkDescriptorSetLayout global_layouts[3] = {internal_shader->descriptor_set_layouts[DESC_SET_INDEX_GLOBAL],
...
b8 vulkan_renderer_shader_instance_resources_acquire(renderer_plugin* plugin, 
    ...
    *out_instance_id = INVALID_ID;
    ...
    VkDescriptorSetLayout layouts[3] = {internal->descriptor_set_layouts[DESC_SET_INDEX_INSTANCE],
...
b8 vulkan_renderpass_create(renderer_plugin* plugin, const renderpass_config* config, renderpass* out_renderpass) {
        ...
        if(attachment_config->type == RENDER_TARGET_ATTACHMENT_TYPE_COLOR) {
            ...
            if(attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_DONT_CARE) {
                ...
                if(attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_LOAD) {
                    if(do_clear_color) {
                        KWARN("Color attachment load operation set to load, but is also set to clear. This combination is invalid, and will err toward clearing. Verify attachment configuration.");
    ...
    if(color_attachment_count > 0) {
        color_attachment_references = kallocate(sizeof(VkAttachmentReference) * color_attachment_count, MEMORY_TAG_ARRAY);
	...
	if(color_attachment_references) {
        kfree(color_attachment_references, sizeof(VkAttachmentReference) * color_attachment_count, MEMORY_TAG_ARRAY);
    }
...
texture* vulkan_renderer_window_attachment_get(renderer_plugin* plugin, u8 index) {
    ...
    if(index >= context->swapchain.image_count) {
        KFATAL("Attempting to get color attachment index out of range: %d. Attachment count: %d", index, context->swapchain.image_count);
        return 0;
    }
...
static b8 vulkan_buffer_copy_range_internal(vulkan_context* context, VkBuffer source, u64 source_offset, VkBuffer dest, u64 dest_offset, u64 size) {
    ...
    vulkan_command_buffer_begin_single_use(context, context->device.graphics_command_pool, &temp_command_buffer);
...
b8 vulkan_buffer_copy_range(renderer_plugin* plugin, renderbuffer* source, u64 source_offset, renderbuffer* dest, u64 dest_offset, u64 size) {
    ...
    if(!source || !source->internal_data || !dest || !dest->internal_data || !size) {
        KERROR("vulkan_buffer_copy_range requires valid pointers to source and destination buffers as well as a nonzero size.");
        return false;
    }

    return vulkan_buffer_copy_range_internal(context,
