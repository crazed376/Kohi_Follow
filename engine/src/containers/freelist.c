...
void freelist_create(u64 total_size, u64* memory_requirement, void* memory, freelist* out_list) {
    ...
    // Catch an edge case of having a really small amount of memory to manage, ond only having a
        ...
        if(node->size == size) {
            ...
            *out_offset = node->offset;
            ...
            if(previous) {
                ...
                // This node is the head of the list. Reassign
                // the head and return the previous head node.
            ...
            // Node is larger. Deduct the memory from it and move
    ...
    if(!node) {
        ...
        while(node) {
            if(node->offset + node->size == offset) {
                ...
                // If there is a previous node, then new node should be inserted between this and it
                if(previous) {
    ...
    if(!old_node) {
        ...
        while(old_node) {
            ...
            if(old_node->next) {
                ...
                // just append to the size. Otherwise, create a new node
                // and attach to it.
