...
static b8 material_loader_load(struct resource_loader* self, const char* name, void* params, resource* out_resource) {
    ...
    while(filesystem_read_line(&f, 511, &p, &line_length)) {
        ...
        // Parse section tags
        if(trimmed[0] == '[') {
        	// Section tag - determin if opening or closing
        	if(trimmed[1] == '/') {
        		// Closing tag
        		switch(parse_mode) {
        			default:
        ...
        // Assume a max of 446 (511-65) for the max length of the value to account for the variable name and the '='
        char raw_value[446];
        ...
        if(strings_equali(trimmed_var_name, "version")) {
			...
			if(resource_data->version == 1) {
				...
				KERROR("Format error: Unexpected variable 'diffuse_map_name', this should only exist for version 1 materials. Ignored.");
			}
        ...
        } else if(strings_equali(trimmed_var_name, "diffuse_color") || strings_equali(trimmed_var_name, "diffuse_colour")) {
			if(resource_data->version == 1) {
				...
				KERROR("Format error: Unexpected variable 'diffuse_color', this should only exist for version 1 materials. Ignored.");
			}
        	...
        	if(resource_data->version == 1) {
				...
				KERROR("Format error: Unexpected variable 'shininess', this should only exist for version 1 materials. Ignored.");
			}
			...
			if(resource_data->version >= 2) {
				...
				KERROR("Format error: Unexpected variable 'type', this should only exist for version 2+ materials.");
			}
