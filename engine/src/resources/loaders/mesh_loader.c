...
static b8 mesh_loader_load(struct resource_loader* self, const char* name, void* params, resource* out_resource) {
    ...
    // binary versions) for debug purpose.
...
static b8 import_obj_file(file_handle* obj_file, const char* out_ksm_filename, geometry_config** out_geometries_darray) {
    ...
    for(u64 i=0; i<count; ++i) {
		...
		// Also generates tangents here, this way tangents are also stored in the output file.
...
// existing material name would be used, which would visually be wrong and sever as
// additional reinforcement of the message for material uniqueness.
