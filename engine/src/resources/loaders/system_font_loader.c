...
static b8 system_font_loader_load(struct resource_loader* self, const char* name, void* params, resource* out_resource) {
    ...
    // This is to prioritize the loading of a binary version of the system font, 
    // followed by importing various types of system fonts to binary types, which 
    // would be loaded on the next run.
