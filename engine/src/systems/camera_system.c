...
b8 camera_system_initialize(u64* memory_requirement, void* state, void* config) {
    ...
    *memory_requirement = struct_requirement + array_requirement + hashtable_requirement;
    ...
    // The array block is after that state. Already allocated, so just set the pointer
    ...
    // Fill the hashtable with invalid referernces to use as a default
...
camera* camera_system_acquire(const char* name) {
    ...
    KERROR("camera_system_acquire called before system initialization. Null returned.");
    return 0;
}
