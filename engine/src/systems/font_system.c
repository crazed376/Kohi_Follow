...
b8 font_system_system_font_load(system_font_config* config) {
    ...
    // Loop through the faces and create one lookup for each, as well as a default
    // size variant for each lookup
...
b8 font_system_bitmap_font_load(bitmap_font_config* config) {
    ...
    // Acquire the texture
    // TODO: only accouts for one page at the moment
...
static b8 setup_font_data(font_data* font) {
    ...
    // x_advance and just use that. If there is not, then create one based off space x 4
    if(!font->tab_x_advance) {
        ...
        if(!font->tab_x_advance) {
            ...
            if(!font->tab_x_advance) {
                // If _still_ not there, then a space wasn't present either,
                // so just hardcode something, in this case font size * 4
...
static b8 rebuild_system_font_variant_atlas(system_font_lookup* lookup, font_data* variant) {
    ...
    // Begin packing all known characters into the atlas. This creates
    // a single-channel image with rendered glyphs at the given size
