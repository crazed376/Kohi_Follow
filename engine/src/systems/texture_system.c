...
texture* texture_system_acquire(const char* name, b8 auto_release) {
    // Returns defualt texture, but warn about it since this should be returned via get_default_texture();
...
texture* texture_system_acquire_writeable(const char* name, u32 width, u32 height, u8 channel_count, b8 has_transparency) {
    ...
    // resources are created and managed somewhere withing the renderer internals
...
static b8 create_default_textures(texture_system_state* state) {
    ...
    // Default diffuce map is all white
    ...
    *out_texture_id = INVALID_ID;
    if(state_ptr) {
        ...
        if(hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
            // If the reference count start off at zero, oe of two things can be
            ...
            if(reference_diff < 0) {
                ...
                if(ref.reference_count == 0 && ref.auto_release) {
                    ...
                    // KTRACE("Released texture '%s'. Texture unloaded because reference_count=0 and auto_release=true.", name_copy);
                } else {
                    // KTRACE("Released texture '%s', now has a reference_count of '%i' (auto_release=%s).", name_copy, ref.reference_count, ref.auto_release ? "true" : "false");
                }
            } else {
