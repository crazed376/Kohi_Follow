...
u32 identifier_acquire_new_id(void* owner) {
    if(!owners) {
...
void identifier_release_id(u32 id) {
    if(!owners) {
        KERROR("identifier_release_id called before initialization. identifier_acquire_new_id should have been called first. Nothing was done.");
        return;
    }
