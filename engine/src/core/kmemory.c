...
b8 memory_system_initialize(memory_system_configuration config) {
    ...
    if(!dynamic_allocator_create(config.total_alloc_size,
...
void* kallocate_aligned(u64 size, u16 alignment, memory_tag tag) {
    ...
    // The latter shouldn't ever really happen.
    void* block = 0;
    if(state_ptr) {
...
void kfree_aligned(void* block, u64 size, u16 alignment, memory_tag tag) {
    ...
    if(state_ptr) {
        ...
        // before this system was started up. Since this absolutely should be an exceptio
        // to the rule, try freeing it on the platform level. If this fails, some other
        // brand of skullduggery is afoot, and we have bigger problems on our hands.
        if(!result) {
            // TODO: memory alignment
