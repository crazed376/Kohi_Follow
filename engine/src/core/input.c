...
void input_process_key(keys key, b8 pressed) {
    ...
    if(state_ptr && state_ptr->keyboard_current.keys[key] != pressed) {
		...
		for(i32 m=map_count-1; m>=0; --m) {
			...
			// If an unset is detected of the map is marked to override all, stop processing
			if(unset || map->overrides_all) {
				break;
			}
		}
...
b8 input_keymap_pop(void) {
	if(state_ptr) {
		// Pop the keymap from the stack, than re-apply the stack
		keymap popped;
