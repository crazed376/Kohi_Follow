...
b8 platform_system_startup(u64* memory_requirement, void* state, void* config) {
    ...
    *memory_requirement = sizeof(platform_state);
    ...
    // If initially minimized, use SW_MINIMIZE : SWSHOWMINNOACTIVE;
...
b8 kthread_create(pfn_thread_start start_function_ptr, void* params, b8 auto_detach, kthread* out_thread) {
    ...
    out_thread->internal_data = CreateThread(0, 0,                                        // Default stack size
                                             (LPTHREAD_START_ROUTINE)start_function_ptr,  // function ptr
                                             params,                                      // param to pass to thread
                                             0, (DWORD*)&out_thread->thread_id);
...
LRESULT CALLBACK win32_process_message(HWND hwnd, u32 msg, WPARAM w_param, LPARAM l_param) {
    switch (msg) {
        ...
        case WM_SYSKEYUP: {
            ...
            // Keypress only determins if any alt/ctrl/shift key is pressed.
            ...
            // Pass the the input subsystem for processing
            input_process_key(key, pressed);

            // Return 0 to prevent default window behavior for some keypresses, such as alt
            return 0;
        }
        case WM_MOUSEMOVE: {
            // Mouse move
        ...
        case WM_MOUSEWHEEL: {
            ...
            if(z_delta != 0) {
                // Flatten the input to on OS independent (-1, 1)
