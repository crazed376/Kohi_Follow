...
/**
 * If func returns false, closes the provided file handle and logs an error.
 * Also returns false, so the calling function must return a boolean. Calling
 * file must #include "core/logger.h".
 * @param func The function whose result is a boolean
 * @param handle The file handle to be closed on a false function result
 * @return False if the provided function fails.
 */
 ...
 * @param out_data A pointer to a number which will be populated by this method.
 ...
 * @param handle A pointer to a file_handle structure.
 * @param out_bytes A byte array whcih will be populated by this method.
 * @param out_bytes_read A pointer to a number which will be populated with the number of bytes actually read from the file.
 * @return True if successful, otherwise false.
 */
