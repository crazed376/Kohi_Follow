...
static void create_gizmo_mode_none(editor_gizmo* gizmo) {
	...
	data->vertices = kallocate(sizeof(color_vertex_3d) * data->vertex_count, MEMORY_TAG_ARRAY);
	...
	if(gizmo->interaction == EDITOR_GIZMO_INTERACTION_TYPE_MOUSE_DRAG) {
		if(gizmo->mode == EDITOR_GIZMO_MODE_ROTATE) {
			...
			if(gizmo->orientation == EDITOR_GIZMO_ORIENTATION_GLOBAL) {
				// Reset rotation. Will be applied to selection alread
				transform_rotation_set(&gizmo->xform, quat_identity());
			}
		}
	}
...
void editor_gizmo_handle_interaction(editor_gizmo* gizmo, struct camera* c, struct ray* r, editor_gizmo_interaction_type interaction_type) {
	...
	if(gizmo->mode == EDITOR_GIZMO_MODE_MOVE) {
		if(interaction_type == EDITOR_GIZMO_INTERACTION_TYPE_MOUSE_DRAG) {
			...
			// Loop through each axis/axis combo. Loop backwards to give 
			// priority to combos since those hit boxes are much smaller
			mat4 gizmo_world = transform_world_get(&gizmo->xform);
			for(i32 i=6; i>-1; --i) {
		...
		if(interaction_type == EDITOR_GIZMO_INTERACTION_TYPE_MOUSE_DRAG) {
			...
			// product between the direction toward the intersection
			// from the origin, then taking its sign
			f32 d = ksign(vec3_dot(direction_t, dir_from_origin));
			
			// Calculate the scale difference by taking the signed
			// magnitude and scaling the untransformed direction by it
			scale = vec3_mul_scalar(direction, d * dist);
			
			// For global transforms, get the inverse of the rotation and
			// apply that to scale on absolute (global) axes instead of local
			if(gizmo->orientation == EDITOR_GIZMO_ORIENTATION_GLOBAL) {
				if(gizmo->selected_xform) {
			...
			if(gizmo->selected_xform) {
				...
				// Apply scale, bot only on axes that have changed
				for(u8 i=0; i<3; ++i) {
		...
		if(interaction_type == EDITOR_GIZMO_INTERACTION_TYPE_MOUSE_DRAG) {
			...
			// Get the difference in angle between this interaction and the
			// last and use that as the axis angle for rotation
			...
			// NOTE: Also check for NaN, which can be done because floats have
			// a unique property that (x != x) detects NaN
