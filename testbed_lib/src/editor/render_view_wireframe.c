	...
	if(self) {
		...
		for(u32 s=0; s<2; ++s) {
			...
			if(!shader_system_uniform_set_by_index(info->locations.view, &packet->view_matrix)) {
				KERROR("Failed to set the view matrix uniform on wireframe shader.");
				return false;
			}
			...
			if(array) {
				for(u32 i=0; i<counts[s]; ++i) {
					...
					// Selecting the instance allows easy color changing
					...
					if(needs_update) {
						if(!shader_system_uniform_set_by_index(info->locations.color, &inst->color)) {
							KERROR("Unable to set uniform color for wireframe shader.");
							return false;
						}
					}
