...
b8 render_view_pick_on_registered(struct render_view* self) {
    if(self) {
        ...
        kzero_memory(&data->color_target_attachment_texture, sizeof(texture));
...
b8 render_view_pick_on_packet_build(const struct render_view* self, struct frame_data* p_frame_data, struct viewport* v, struct camera* c, void* data, struct render_view_packet* out_packet) {
    ...
    // TODO: this needs to take into account the height id, not the count, because they can and do skip ids
    ...
    if(p_frame_data->render_target_index == 0) {
        ...
        for(u32 i=0; i<world_geometry_count; ++i) {
            ...
            if(!shader_system_uniform_set_by_index(data->world_shader_info.id_color_location, &id_color)) {
                KERROR("Failed to apply id color uniform.");
                return false;
            }
        ...
        // Throwing an error if we try to use in the meantime
