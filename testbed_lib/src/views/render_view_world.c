...
// A private structure used to sort geometry by distance from the camera
...
b8 render_view_world_on_registered(struct render_view* self) {
    if(self) {
		...
		if(!resource_system_load(color3d_shader_name, RESOURCE_TYPE_SHADER, 0, &color3d_shader_config_resource)) {
			KERROR("Failed to load builtin color3d shader resource.");
			return false;
		}
		shader_config* color3d_shader_config = (shader_config*)color3d_shader_config_resource.data;
		// NOTE: Assuming the first pass sinze that's all this view has
		if(!shader_system_create(&self->passes[0], color3d_shader_config)) {
			KERROR("Failed to load builtin color3d shader.");
			return false;
		}
		...
		// Get color3d shader uniform locations
		{
			...
			if(!s) {
				KERROR("Unable to get color3d shader!");
				return false;
			}
...
b8 render_view_world_on_render(const struct render_view* self, const struct render_view_packet* packet, u64 frame_number, u64 render_target_index, const struct frame_data* p_frame_data) {
    ...
    for(u32 p=0; p<self->renderpass_count; ++p) {
		...
		if(terrain_count > 0) {
			...
			// Apply globals
			// TODO: Find a generic way to request data such as ambient color (which should be from a scene) and mode (from the renderer)
		...
		if(debug_geometry_count > 0) {
	     	...
	     	if(!s) {
	     		KERROR("Unable to obtain color3d shader.");
	     		return false;
	     	}
	     	shader_system_use_by_id(s->id);
