...
void main() {
    ...
    if(in_mode == 0 || in_mode == 1) {
        ...
        out_color = calculate_directional_light(instance_ubo.dir_light, normal, view_direction);

		for(int i=0; i<instance_ubo.num_p_lights; ++i) {
	        out_color += calculate_point_light(instance_ubo.p_lights[i], normal, in_dto.frag_position, view_direction);
		}
        // out_color += calculate_point_light(instance_ubo.p_light_1, normal, in_dto.frag_position, view_direction);
    } else if(in_mode == 2) {
...
vec4 calculate_directional_light(directional_light light, vec3 normal, vec3 view_direction) {
    ...
    vec4 ambient = vec4(vec3(in_dto.ambient * instance_ubo.properties.diffuse_color), diff_samp.a);
